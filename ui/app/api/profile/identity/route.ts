...
export const GET = async (req: NextRequest) => {
  return authenticatedHandler(req, async (wallet) => {
    const identity = await identityEngine.getIdentity(await resolveUserId(wallet));
...
export const POST = async (req: NextRequest) => {
  return authenticatedHandler(req, async (wallet, req: NextRequest) => {
    try {
      ...
      const result = await identityEngine.updateIdentity(await resolveUserId(wallet), body);
